...
import { ITV_DEBATE } from "../../interviewConfig";
...
export default function CompanyDebate() {
  const go = (url) => { window.location.href = url; };

  const classPhotos = [
    { slot: "interview_debate1", caption: "토론 발언 코칭" },
    { slot: "interview_debate2", caption: "집단토론 실전" },
    { slot: "interview_debate3", caption: "PT 발표 훈련" },
    { slot: "interview_debate4", caption: "모의면접 현장" },
    { slot: "interview_debate5", caption: "그룹 실전 연습" },
    { slot: "interview_debate6", caption: "비디오 피드백" },
  ];

  const rows = useMemo(() => {
    const all = buildPassList(300, ITV_DEBATE.passSchools);
  ...
  return (
    <div>
      ...
      <section className="relative flex min-h-[420px] items-center justify-center overflow-hidden bg-gradient-to-br from-seum-navy via-[#22356e] to-seum-blue">
        ...
        <div className="relative z-10 mx-auto max-w-4xl px-6 pb-20 pt-28 text-center text-white md:pt-32">
          <span className="mb-5 inline-block rounded-full bg-seum-blue/90 px-4 py-1.5 text-sm font-bold tracking-wide">
            {ITV_DEBATE.hero.badge}
          </span>
          <h1 className="text-3xl font-black sm:text-4xl md:text-5xl">{ITV_DEBATE.hero.title}</h1>
          <p className="mt-3 text-xl font-bold text-seum-blue sm:text-2xl">
            <span className="text-white">{ITV_DEBATE.hero.slogan}</span>
          </p>
          <p className="mx-auto mt-6 max-w-2xl text-sm leading-relaxed text-white/80 sm:text-base">
            {ITV_DEBATE.hero.desc}
          </p>
      ...
      <section className="bg-white py-20">
        <div className="mx-auto max-w-5xl px-6">
          <div className="text-center">
            <h2 className="text-2xl font-black text-seum-navy sm:text-3xl">토론·PT 면접, 이렇게 진행됩니다</h2>
          ...
          <div className="mt-12 flex flex-col items-center gap-4 md:flex-row md:items-stretch md:justify-center">
            {ITV_DEBATE.examFlow.map((f, i) => (
              <div key={i} className="flex w-full flex-col items-center md:w-auto md:flex-1 md:flex-row md:items-stretch">
                ...
                {i < ITV_DEBATE.examFlow.length - 1 && (
      ...
      <section className="bg-slate-50 py-20">
        <div className="mx-auto max-w-5xl px-6">
          <div className="text-center">
            <h2 className="text-2xl font-black text-seum-navy sm:text-3xl">세움면접 토론·PT가 다른 이유</h2>
          </div>
          <div className="mt-12 grid grid-cols-1 gap-5 md:grid-cols-2">
            {ITV_DEBATE.strengths.map((s, i) => (
      ...
      <section className="bg-[#0f1629] py-20">
        <div className="mx-auto max-w-5xl px-6">
          <div className="text-center text-white">
            <p className="text-sm font-bold tracking-widest text-red-500">{ITV_DEBATE.passHeadline}</p>
            <h2 className="mt-3 text-4xl font-black text-red-500 sm:text-6xl">{ITV_DEBATE.passCount}명 합격</h2>
            <p className="mx-auto mt-4 max-w-2xl text-sm text-white/60 sm:text-base">{ITV_DEBATE.passSub}</p>
          </div>
        </div>
      ...
      <section className="bg-slate-50 py-20">
        <div className="mx-auto max-w-5xl px-6">
          <div className="text-center">
            <h2 className="text-2xl font-black text-seum-navy sm:text-3xl">토론·PT 면접 커리큘럼 (총 6강)</h2>
            <p className="mt-3 text-sm text-slate-500 sm:text-base">그룹수업 1회 2시간 · 6명 정원 / 1:1 개인지도는 90분 과정으로도 가능합니다.</p>
          </div>
          <div className="mt-12 grid grid-cols-1 gap-4 sm:grid-cols-2">
            {ITV_DEBATE.curriculum.map((c, i) => (
      ...
      <section className="bg-seum-navy py-16">
        <div className="mx-auto max-w-3xl px-6 text-center text-white">
          <h2 className="text-2xl font-black sm:text-3xl">혼자서 토론·PT 면접을 준비하고 있나요?</h2>
          <p className="mt-4 text-sm text-white/70 sm:text-base">집단토론 발언 전략부터 PT 구성·발표까지. 세움면접이 합격까지 함께합니다.</p>
