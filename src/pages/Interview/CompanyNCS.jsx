...
import { ITV_NCS } from "../../interviewConfig";
...
export default function CompanyNCS() {
  const go = (url) => { window.location.href = url; };

  const classPhotos = [
    { slot: "interview_ncs1", caption: "경험면접 STAR 코칭" },
    { slot: "interview_ncs2", caption: "상황면접 훈련" },
    { slot: "interview_ncs3", caption: "자소서 심층 대비" },
    { slot: "interview_ncs4", caption: "모의면접 현장" },
    { slot: "interview_ncs5", caption: "그룹 실전 연습" },
    { slot: "interview_ncs6", caption: "비디오 피드백" },
  ];

  const rows = useMemo(() => {
    const all = buildPassList(300, ITV_NCS.passSchools);
    ...
    <div>
      ...
      <section className="relative flex min-h-[420px] items-center justify-center overflow-hidden bg-gradient-to-br from-seum-navy via-[#22356e] to-seum-blue">
        ...
        <div className="relative z-10 mx-auto max-w-4xl px-6 pb-20 pt-28 text-center text-white md:pt-32">
          ...
          <h1 className="text-3xl font-black sm:text-4xl md:text-5xl">{ITV_NCS.hero.title}</h1>
          <p className="mt-3 text-xl font-bold text-seum-blue sm:text-2xl">
            <span className="text-white">{ITV_NCS.hero.slogan}</span>
          </p>
      ...
      <section className="bg-white py-20">
        <div className="mx-auto max-w-5xl px-6">
          <div className="text-center">
            <h2 className="text-2xl font-black text-seum-navy sm:text-3xl">NCS·직무역량 면접, 이렇게 진행됩니다</h2>
      ...
      <section className="bg-slate-50 py-20">
        <div className="mx-auto max-w-5xl px-6">
          <div className="text-center">
            <h2 className="text-2xl font-black text-seum-navy sm:text-3xl">세움면접 NCS·직무 면접이 다른 이유</h2>
          </div>
          <div className="mt-12 grid grid-cols-1 gap-5 md:grid-cols-2">
            {ITV_NCS.strengths.map((s, i) => (
      ...
      <section className="bg-[#0f1629] py-20">
        <div className="mx-auto max-w-5xl px-6">
          <div className="text-center text-white">
            <p className="text-sm font-bold tracking-widest text-red-500">{ITV_NCS.passHeadline}</p>
            <h2 className="mt-3 text-4xl font-black text-red-500 sm:text-6xl">{ITV_NCS.passCount}명 합격</h2>
            <p className="mx-auto mt-4 max-w-2xl text-sm text-white/60 sm:text-base">{ITV_NCS.passSub}</p>
          </div>
        </div>
      ...
      <section className="bg-seum-navy py-16">
        <div className="mx-auto max-w-3xl px-6 text-center text-white">
          <h2 className="text-2xl font-black sm:text-3xl">혼자서 NCS·직무 면접을 준비하고 있나요?</h2>
          <p className="mt-4 text-sm text-white/70 sm:text-base">경험·상황·직무 면접부터 자소서 심층 대비까지. 세움면접이 합격까지 함께합니다.</p>
