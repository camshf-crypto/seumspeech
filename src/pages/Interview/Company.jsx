...
import { ITV_COMPANY } from "../../interviewConfig";
...
export default function Company() {
  ...
  const classPhotos = [
    { slot: "interview_company1", caption: "경험면접 코칭" },
    { slot: "interview_company2", caption: "집단토론 실전" },
    { slot: "interview_company3", caption: "PT 발표 훈련" },
    { slot: "interview_company4", caption: "모의면접 현장" },
    { slot: "interview_company5", caption: "그룹 실전 연습" },
    { slot: "interview_company6", caption: "비디오 피드백" },
  ];

  const rows = useMemo(() => {
    const all = buildPassList(300, ITV_COMPANY.passSchools);
  ...
  return (
    <div>
      ...
      <section className="relative flex min-h-[420px] items-center justify-center overflow-hidden bg-gradient-to-br from-seum-navy via-[#22356e] to-seum-blue">
        ...
        <div className="relative z-10 mx-auto max-w-4xl px-6 pb-20 pt-28 text-center text-white md:pt-32">
          <span className="mb-5 inline-block rounded-full bg-seum-blue/90 px-4 py-1.5 text-sm font-bold tracking-wide">
            {ITV_COMPANY.hero.badge}
          </span>
          <h1 className="text-3xl font-black sm:text-4xl md:text-5xl">{ITV_COMPANY.hero.title}</h1>
          <p className="mt-3 text-xl font-bold text-seum-blue sm:text-2xl">
            <span className="text-white">{ITV_COMPANY.hero.slogan}</span>
          </p>
          <p className="mx-auto mt-6 max-w-2xl text-sm leading-relaxed text-white/80 sm:text-base">
            {ITV_COMPANY.hero.desc}
          </p>
      ...
      <section className="bg-white py-20">
        <div className="mx-auto max-w-5xl px-6">
          <div className="text-center">
            <h2 className="text-2xl font-black text-seum-navy sm:text-3xl">공기업 면접, 이렇게 진행됩니다</h2>
          ...
          <div className="mt-12 flex flex-col items-center gap-4 md:flex-row md:items-stretch md:justify-center">
            {ITV_COMPANY.examFlow.map((f, i) => (
              <div key={i} className="flex w-full flex-col items-center md:w-auto md:flex-1 md:flex-row md:items-stretch">
                ...
                {i < ITV_COMPANY.examFlow.length - 1 && (
      ...
      <section className="bg-slate-50 py-20">
        <div className="mx-auto max-w-5xl px-6">
          <div className="text-center">
            <h2 className="text-2xl font-black text-seum-navy sm:text-3xl">세움면접 공기업과정이 다른 이유</h2>
          </div>
          <div className="mt-12 grid grid-cols-1 gap-5 md:grid-cols-2">
            {ITV_COMPANY.strengths.map((s, i) => (
      ...
      <section className="bg-[#0f1629] py-20">
        <div className="mx-auto max-w-5xl px-6">
          <div className="text-center text-white">
            <p className="text-sm font-bold tracking-widest text-red-500">{ITV_COMPANY.passHeadline}</p>
            <h2 className="mt-3 text-4xl font-black text-red-500 sm:text-6xl">{ITV_COMPANY.passCount}명 합격</h2>
            <p className="mx-auto mt-4 max-w-2xl text-sm text-white/60 sm:text-base">{ITV_COMPANY.passSub}</p>
          </div>
        </div>
      ...
      <section className="bg-slate-50 py-20">
        <div className="mx-auto max-w-5xl px-6">
          ...
          <div className="mt-12 grid grid-cols-1 gap-5 sm:grid-cols-2">
            {ITV_COMPANY.classTypes.map((c, i) => (
      ...
      <section className="bg-slate-50 py-20">
        <div className="mx-auto max-w-5xl px-6">
          <div className="text-center">
            <h2 className="text-2xl font-black text-seum-navy sm:text-3xl">공기업 면접 커리큘럼 (총 6강)</h2>
            <p className="mt-3 text-sm text-slate-500 sm:text-base">그룹수업 1회 2시간 · 6명 정원 / 1:1 개인지도는 90분 과정으로도 가능합니다.</p>
          </div>
          <div className="mt-12 grid grid-cols-1 gap-4 sm:grid-cols-2">
            {ITV_COMPANY.curriculum.map((c, i) => (
      ...
      <section className="bg-seum-navy py-16">
        <div className="mx-auto max-w-3xl px-6 text-center text-white">
          <h2 className="text-2xl font-black sm:text-3xl">혼자서 공기업 면접을 준비하고 있나요?</h2>
          <p className="mt-4 text-sm text-white/70 sm:text-base">공사별 맞춤 전략과 토론·PT 실전 대비까지. 세움면접이 합격까지 함께합니다.</p>
