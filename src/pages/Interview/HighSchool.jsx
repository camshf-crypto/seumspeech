...
import { ITV_HIGH } from "../../interviewConfig";
import { useSiteImage } from "../../lib/useSiteImage";

// 수업 사진 한 장 (site_images 의 interview_highN 슬롯에서 불러옴)
  ...
  return (
    <div className="flex overflow-hidden">
      <div
        ...
        style={{
          animation: `${reverse ? "passmarquee-rev" : "passmarquee"} ${duration}s linear infinite`,
        }}
      >
        {loop.map((p, i) => (
          <div
...
export default function HighSchool() {
  ...
  // 수업 현장 사진 6장
  const classPhotos = [
    { slot: "interview_high1", caption: "자소서 1:1 첨삭" },
    { slot: "interview_high2", caption: "학교별 기출 분석" },
    { slot: "interview_high3", caption: "AI 모의면접 연습" },
    { slot: "interview_high4", caption: "제시문 면접 훈련" },
    { slot: "interview_high5", caption: "소수정예 그룹 수업" },
    { slot: "interview_high6", caption: "실전 모의면접 촬영" },
  ];

  // 합격자 300명 자동 생성 후 3줄로 분할
  const rows = useMemo(() => {
    const all = buildPassList(ITV_HIGH.passCount, ITV_HIGH.passSchools);
    ...
    <div>
      ...
      <section className="relative flex min-h-[420px] items-center justify-center overflow-hidden bg-gradient-to-br from-seum-navy via-[#22356e] to-seum-blue">
        ...
        <div className="relative z-10 mx-auto max-w-4xl px-6 pb-20 pt-28 text-center text-white md:pt-32">
          ...
          <h1 className="text-3xl font-black sm:text-4xl md:text-5xl">{ITV_HIGH.hero.title}</h1>
          <p className="mt-3 text-xl font-bold text-seum-blue sm:text-2xl">
            <span className="text-white">{ITV_HIGH.hero.slogan}</span>
          </p>
          ...
          <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
            <button
              onClick={() => go("/consult")}
              className="rounded-xl bg-white px-7 py-3.5 text-base font-bold text-seum-navy shadow-lg transition hover:bg-slate-100"
            >
              무료 상담 신청
            </button>
            <button
      ...
      <section className="bg-white py-20">
        <div className="mx-auto max-w-5xl px-6">
          <div className="text-center">
            <h2 className="text-2xl font-black text-seum-navy sm:text-3xl">
              세움면접 고입과정이 다른 이유
            </h2>
          </div>
          <div className="mt-12 grid grid-cols-1 gap-5 md:grid-cols-2">
            {ITV_HIGH.strengths.map((s, i) => (
              <div
                key={i}
              ...
              >
                <span className="inline-block rounded-md bg-seum-navy px-3 py-1 text-[11px] font-bold tracking-wide text-white">
                  {s.tag}
                </span>
      ...
      <section className="bg-[#0f1629] py-20">
        <div className="mx-auto max-w-5xl px-6">
          <div className="text-center text-white">
            <p className="text-sm font-bold tracking-widest text-red-500">합격의 신화를 세우다!!</p>
            <h2 className="mt-3 text-4xl font-black text-red-500 sm:text-6xl">
              {ITV_HIGH.passCount}명 합격
            </h2>
        ...
        <div className="mx-auto max-w-5xl px-6">
          <div className="text-center text-white">
            <h2 className="text-2xl font-black sm:text-3xl">
              세움 자체 면접 프로그램으로 실전 연습
            </h2>
            <p className="mx-auto mt-4 max-w-2xl text-sm text-white/70 sm:text-base">
              인성면접부터 제시문·꼬리질문까지, 실제 면접장처럼 반복 훈련하는
              세움면접만의 AI 도구를 수업에 함께 활용합니다.
            </p>
          </div>
          <div className="mt-12 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {ITV_HIGH.aiFeatures.map((f, i) => (
              <div
                key={i}
                className="flex items-center gap-3 rounded-xl bg-white/10 p-5 ring-1 ring-white/15"
              >
                <span className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-seum-blue text-sm font-bold text-white">
                  {i + 1}
                </span>
      ...
      <section className="bg-white py-20">
        <div className="mx-auto max-w-5xl px-6">
          ...
          <div className="mt-12 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {ITV_HIGH.targets.map((t, i) => (
              <div
                key={i}
                className="flex items-start gap-3 rounded-xl border border-slate-100 bg-slate-50 p-5"
              >
                <span className="mt-0.5 flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-seum-blue/10 text-xs font-bold text-seum-blue">
                  ✓
                </span>
      ...
      <section className="bg-slate-50 py-20">
        <div className="mx-auto max-w-5xl px-6">
          ...
          <div className="mt-12 space-y-4">
            {ITV_HIGH.system.map((s, i) => (
              <div
                key={i}
                className="flex items-center gap-5 rounded-2xl border border-slate-100 bg-white p-6 shadow-sm"
              >
                <span className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-seum-navy text-lg font-black text-white">
                  {s.step}
                </span>
                <div>
      ...
      <section className="bg-white py-20">
        <div className="mx-auto max-w-5xl px-6">
          <div className="text-center">
            <h2 className="text-2xl font-black text-seum-navy sm:text-3xl">고입 면접 8주 커리큘럼</h2>
          </div>
          <div className="mt-12 grid grid-cols-1 gap-4 sm:grid-cols-2">
            {ITV_HIGH.curriculum.map((c, i) => (
      ...
      <section className="bg-white py-20">
        <div className="mx-auto max-w-5xl px-6">
          ...
          <div className="mx-auto mt-12 max-w-3xl space-y-3">
            {ITV_HIGH.effects.map((e, i) => (
              <div
                key={i}
                className="flex items-center gap-4 rounded-xl border border-slate-100 bg-slate-50 p-5"
              >
                <span className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-seum-navy text-sm font-black text-white">
      ...
      <section className="bg-seum-navy py-16">
        <div className="mx-auto max-w-3xl px-6 text-center text-white">
          <h2 className="text-2xl font-black sm:text-3xl">혼자서 고입 면접을 준비하고 있나요?</h2>
          <p className="mt-4 text-sm text-white/70 sm:text-base">
            자소서 데이터부터 AI 면접 시뮬레이션까지, 세움면접이 합격까지 함께합니다.
          </p>
          <button
            onClick={() => go("/consult")}
