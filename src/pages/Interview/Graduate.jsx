...
import { ITV_GRAD } from "../../interviewConfig";
...
export default function Graduate() {
  ...
  const classPhotos = [
    { slot: "interview_grad1", caption: "학업계획서 1:1 분석" },
    { slot: "interview_grad2", caption: "대학원별 기출 분석" },
    { slot: "interview_grad3", caption: "전공 논리 설명 훈련" },
    { slot: "interview_grad4", caption: "AI 모의면접 연습" },
    { slot: "interview_grad5", caption: "1:6 소수정예 수업" },
    { slot: "interview_grad6", caption: "실전 모의면접 촬영" },
  ];

  const rows = useMemo(() => {
    const all = buildPassList(300, ITV_GRAD.passSchools);
    ...
    <div>
      ...
      <section className="relative flex min-h-[420px] items-center justify-center overflow-hidden bg-gradient-to-br from-seum-navy via-[#22356e] to-seum-blue">
        ...
        <div className="relative z-10 mx-auto max-w-4xl px-6 pb-20 pt-28 text-center text-white md:pt-32">
          ...
          <h1 className="text-3xl font-black sm:text-4xl md:text-5xl">{ITV_GRAD.hero.title}</h1>
          <p className="mt-3 text-xl font-bold text-seum-blue sm:text-2xl">
            <span className="text-white">{ITV_GRAD.hero.slogan}</span>
          </p>
      ...
      <section className="bg-white pb-20">
        <div className="mx-auto max-w-5xl px-6">
          <div className="text-center">
            <h2 className="text-2xl font-black text-seum-navy sm:text-3xl">세움면접 대학원과정이 다른 이유</h2>
          </div>
          <div className="mt-12 grid grid-cols-1 gap-5 md:grid-cols-2">
            {ITV_GRAD.strengths.map((s, i) => (
      ...
      <section className="bg-[#0f1629] py-20">
        <div className="mx-auto max-w-5xl px-6">
          <div className="text-center text-white">
            <p className="text-sm font-bold tracking-widest text-red-500">{ITV_GRAD.passHeadline}</p>
            <h2 className="mt-3 text-4xl font-black text-red-500 sm:text-6xl">{ITV_GRAD.passCount}명 합격</h2>
            <p className="mx-auto mt-4 max-w-2xl text-sm text-white/60 sm:text-base">{ITV_GRAD.passSub}</p>
          </div>
        </div>
      ...
      <section className="bg-slate-50 py-20">
        <div className="mx-auto max-w-5xl px-6">
          <div className="text-center">
            <h2 className="text-2xl font-black text-seum-navy sm:text-3xl">{ITV_GRAD.recommend.title}</h2>
            <p className="mt-3 text-sm text-slate-500 sm:text-base">{ITV_GRAD.recommend.desc}</p>
          </div>
          <div className="mt-12 grid grid-cols-1 gap-5 sm:grid-cols-3">
            {ITV_GRAD.recommend.items.map((r, i) => (
      ...
      <section className="bg-seum-navy py-20">
        <div className="mx-auto max-w-5xl px-6">
          <div className="text-center text-white">
            <h2 className="text-2xl font-black sm:text-3xl">대학원 면접, 이렇게 대비합니다</h2>
            <p className="mx-auto mt-4 max-w-2xl text-sm text-white/70 sm:text-base">
              학업계획서 기반 예상질문부터 전공 논리 설명, 모의면접까지. 세움면접의 체계적인 면접 훈련입니다.
            </p>
          </div>
          <div className="mt-12 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {ITV_GRAD.aiFeatures.map((f, i) => (
      ...
      <section className="bg-white py-20">
        <div className="mx-auto max-w-5xl px-6">
          <div className="text-center">
            <h2 className="text-2xl font-black text-seum-navy sm:text-3xl">대학원 면접 6주 커리큘럼</h2>
          </div>
          <div className="mt-12 grid grid-cols-1 gap-4 sm:grid-cols-2">
            {ITV_GRAD.curriculum.map((c, i) => (
      ...
      <section className="bg-slate-50 py-20">
        <div className="mx-auto max-w-5xl px-6">
          <div className="text-center">
            ...
            <p className="mt-3 text-sm text-slate-500 sm:text-base">세움면접 대학원과정의 실제 수업 모습입니다.</p>
          </div>
      ...
      <section className="bg-seum-navy py-16">
        <div className="mx-auto max-w-3xl px-6 text-center text-white">
          <h2 className="text-2xl font-black sm:text-3xl">혼자서 대학원 면접을 준비하고 있나요?</h2>
          <p className="mt-4 text-sm text-white/70 sm:text-base">학업계획서 분석부터 전공 논리 훈련까지. 세움면접이 합격까지 함께합니다.</p>
