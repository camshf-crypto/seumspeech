...
import { ITV_NAV } from "../../interviewConfig";
        ...
        <div className="mx-auto flex h-14 max-w-5xl items-stretch justify-center px-6">
          {ITV_NAV.map((m, idx) => (
            <button
              key={m.label}
              onClick={() => go(m.href)}
              className={`flex flex-1 items-center justify-center whitespace-nowrap px-3 text-[15px] font-semibold text-slate-700 transition-colors hover:text-seum-blue ${
      ...
      >
        <div className="mx-auto flex max-w-5xl justify-center px-6 pb-10 pt-8">
          {ITV_NAV.map((m, idx) => (
            <ul
              key={m.label}
              className={`flex flex-1 flex-col items-center gap-5 px-3 ${
        ...
        <nav className="border-t border-slate-100 bg-white md:hidden">
          ...
          {ITV_NAV.map((m, i) => (
            <div key={m.label} className="border-b border-slate-50">
              ...
              {mobileSub === i && m.sub && m.sub.length > 0 && (
