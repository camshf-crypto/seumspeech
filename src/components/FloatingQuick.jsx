    ...
    <div className="fixed right-0 top-1/3 z-50 flex -translate-y-1/2">
      {open ? (
        <div className="flex w-24 flex-col items-stretch overflow-hidden rounded-l-xl border-l border-y border-slate-200 bg-white text-seum-navy shadow-2xl">
          ...
          {/* 카카오톡 (새 탭으로 채널 열기) */}
          <button
            onClick={() => window.open("http://pf.kakao.com/_YyATxb", "_blank")}
            className="flex flex-col items-center justify-center gap-2 border-t border-slate-200 bg-[#FEE500] py-5 text-[#3b1e1e] hover:brightness-95"
          >
            <Ico.kakao className="h-7 w-7" />
            <span className="text-[12px] font-bold">카톡상담</span>
          </button>
