...
// 콘텐츠 관리(site_images)에서 올린 지점 사진을 BRANCHES 순서대로 쓴다.
// 첫 번째 지점(마곡점) → room1, 두 번째 지점(루원시티점) → room2
    ...
    <section className="bg-slate-50 py-20">
      <div className="mx-auto max-w-6xl px-6">
        ...
        <div className="mx-auto grid max-w-3xl gap-6 sm:grid-cols-2">
          {BRANCHES.map((b, idx) => {
            // 콘텐츠 관리에서 올린 사진이 우선, 없을 때만 config.js 값을 쓴다
            const src = img(ROOM_SLOTS[idx], "") || b.img;

            return (
              <div key={b.id} className="overflow-hidden rounded-2xl bg-white shadow-sm">
                {/* object-cover: 카드 폭을 꽉 채운다. 비율이 안 맞는 부분은 가운데 기준으로 잘림 */}
                <Img
                  src={src}
                  alt={b.name}
                  label={`${b.name} 사진`}
